...
# 1. 가게별 중첩 메뉴 데이터 구축 (입력값 리스트화 반영)[span_3](start_span)[span_3](end_span)
store_menu_data = {
    "한솥도시락": {
        "치킨마요": 3800, "빅치킨마요": 4400, "돈까스도련님": 4900, 
        "제육볶음": 4700, "진달래도시락": 8000
    },
    "신전떡볶이": {
        "떡볶이(1인분)": 3500, "로제떡볶이": 5500, "치즈김밥": 4000, 
        "튀김오뎅(5개)": 1700, "잡채말이(3개)": 1700
    },
    "써브웨이": {
        "이탈리안BMT": 7400, "에그마요": 6200, "스테이크앤치즈": 8400, 
        "로스트치킨": 7300, "쿠키팩(6개)": 9100
    },
    "맘스터치": {
        "싸이버거세트": 7500, "불불불불싸이버거": 6500, "휠렛버거": 5000, 
        "케이준양념감자(L)": 3500, "치즈스틱": 2000
    },
    "두찜": {
        "까만찜닭(한마리)": 24800, "로제찜닭(한마리)": 27800, "마라로제찜닭": 28800, 
        "납작만두(5개)": 2500, "공기밥": 1000
    },
    "청년피자": {
        "에그콘피자(L)": 24900, "매드쉬림프(L)": 24900, "진짜감자피자(L)": 18900, 
        "할라불고기(L)": 18900, "알리오올리오파스타": 7900
    },
    "메가커피": {
        "아이스아메리카노": 2000, "딸기라떼": 3700, "큐브라떼": 4200, 
        "허니자몽블랙티": 3700, "감자빵": 3500
    },
    "설빙": {
        "인절미설빙": 9500, "애플망고치즈설빙": 13900, "초코브라우니설빙": 12900, 
        "생딸기설빙": 15500, "인절미토스트": 4800
    },
    "미쉐": {
        "소프트아이스크림": 1000, "레몬에이드": 1500, "망고선데이": 2500, 
        "브라운슈가펄밀크티": 3500, "복숭아얼그레이": 2500
    }
}

# 배달비 산정 기준 (중간보고서 기반)[span_4](start_span)[span_4](end_span)
delivery_rules = [
    (50000, 0), (35000, 1000), (18000, 3000), (0, 4000)
]
...
def calculate_total_cost(menu_dict, combo_list, coupon_amount):
    ...
    # 배달비 역전 구간 탐색용 비용 계산[span_5](start_span)[span_5](end_span)
    delivery_fee = 4000
    ...
    # $$TotalCost = FoodPrice + DeliveryFee - Discount$$
...
st.caption("성균관대학교 문제해결과컴퓨팅사고 - 러닝페어 2팀 (김나은, 여샘물, 왕예은, 정보미)")
...
st.info("💡 알고리즘 가동을 위해 아래 동의를 체크해 주세요.")
agree = st.checkbox("개인정보 수집 및 최적화 로직 활용에 동의합니다.")
...
if agree:
    if st.button("🚀 최적의 조합 계산하기"):
        ...
        # 알고리즘: 완전 탐색 기반 최적화[span_7](start_span)[span_7](end_span)
        for i in range(1, 5): 
    ...
    st.warning("동의 체크박스를 선택하셔야 알고리즘이 가동됩니다.")
